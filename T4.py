...
class Config:
    ...
    BATCH_SIZE = 64
    EPOCHS = 200        # Increased epochs to ensure convergence for tighter bottleneck
    LR = 1e-3
    SEED = 42
    INPUT_DIM = 6
    HIDDEN_DIMS = [32, 16, 8] # Tighter bottleneck: 6 -> 4 -> 2 -> 4 -> 6
...
# ============================================================================
# 2. Model Definition: Residual Autoencoder (More Complex)
# ============================================================================
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, dim, dropout=0.2):
        ...
        self.block = nn.Sequential(
            nn.Linear(dim, dim),
            nn.BatchNorm1d(dim),
            nn.SiLU(), # Swish activation often works better
            nn.Dropout(dropout),
            nn.Linear(dim, dim),
            nn.BatchNorm1d(dim)
        )
...
class AdvancedAutoencoder(nn.Module):
    def __init__(self, input_dim):
        ...
        # Encoder: Expand -> Process -> Compress
        self.encoder = nn.Sequential(
            nn.Linear(input_dim, 64), # Wider first layer: 6 -> 64
            nn.BatchNorm1d(64),
            nn.SiLU(),
            ResidualBlock(64, dropout=0.2), # Deeper processing
            ResidualBlock(64, dropout=0.2),
            nn.Linear(64, 16),
            nn.BatchNorm1d(16),
            nn.SiLU(),
            nn.Linear(16, 3) # Latent Dim = 3 (Tighter bottleneck than 4)
        )
...
def analyze_results(train_errors, test_errors, test_labels):
    ...
    # Strategy 1: Statistical (Unsupervised) - e.g., Mean + 1.5 Std
    threshold_stat = train_errors.mean() + 1.5 * train_errors.std()
    
    # Strategy 2: Optimal Threshold from ROC Curve (Supervised Evaluation)
    # This shows the "Best Possible" performance of the model architecture
    fpr, tpr, thresholds = roc_curve(test_labels, test_errors)
    
    # Calculate J statistic (Youden's J statistic) = TPR - FPR
    J = tpr - fpr
    ix = np.argmax(J)
    best_thresh = thresholds[ix]
    
    print("\n--- Anomaly Detection Analysis ---")
    print(f"Statistical Threshold (Mean+1.5*Std): {threshold_stat:.6f}")
    print(f"Optimal Threshold (Best Balance):     {best_thresh:.6f}")
    
    # Use the Optimal Threshold for final evaluation to show model potential
...
def main():
    ...
    # 2. Build Model
    model = AdvancedAutoencoder(Config.INPUT_DIM).to(device)
    # print(model)
